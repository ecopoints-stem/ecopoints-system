...
class SeparatedMaterialPagingSource (
    private val ecoApi: EcoApi,
    private val employeeId: Long
): PagingSource<Int, SeparatedMaterial>(){
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SeparatedMaterial> {
        val page = params.key ?: 0
        return try {
            val response = ecoApi.getAllSpMaterialByEmployeeId(
                employeeId,
                page = page,
    ...
    override fun getRefreshKey(state: PagingState<Int, SeparatedMaterial>): Int? {
