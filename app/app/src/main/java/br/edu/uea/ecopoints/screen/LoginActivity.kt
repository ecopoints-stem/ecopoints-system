    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loginViewModel.state.observe(this){
            ...
            if(state.isAuthenticated && matchesRole(state.auth,userRole)){
                if(state.auth?.isPasswordRecovery != true){
                    when(userRole){
                    ...
                    val resetIntent: Intent = Intent(this@LoginActivity, ResetPasswordActivity::class.java)
                    resetIntent.putExtra("roleSelected",userRole)
                    val password: String = shared.getString("password", "") ?: ""
                    Log.i("ECO","PASSWORD $password")
                    resetIntent.putExtra("tmp",password)
                    resetIntent.flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
